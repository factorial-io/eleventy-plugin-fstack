...
/**
 * This updates the manifest by crawling the buildPath/assetsPaths
...
module.exports = (userOptions) => {
  ...
  const assetsPath = `/${userOptions.assetsPath.base}`;
  const assetsCssPath = path.join(
    process.cwd(),
    buildPath,
    assetsPath,
    `/${userOptions.assetsPath.css}`
  );
  const assetsJsPath = path.join(
    process.cwd(),
    buildPath,
    assetsPath,
    `/${userOptions.assetsPath.js}`
  );
